...
// NEW - Extra Functionality test
    let donation1 = db.Donation.create({
      name: "Martin Chorley",
      amount: "2000"
    })
    let donation2 = db.Donation.create({
      name: "Sam Smith",
      amount: "700"
    })
    let donation3 = db.Donation.create({
      name: "Kevin Goodman",
      amount: "40"
    })

    // wait for all the objects to save and then instantiate relationships.
    Promise.all([book1, book2, book3, author1, author2, author3, user1, loan1, donation1]).then(function(
        results
    ) {
        b1 = results[0];
        b2 = results[1];
